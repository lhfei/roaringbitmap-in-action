...
public class CompressionResultsTest extends BasicTestSuite {
	...
	public void testSuperSparse() {
		...
		r.runOptimize();
		LOG.info("Bits used per value after run optimize = {}", F.format(r.getSizeInBytes() * 8.0 / howmany));
		LOG.info(
				"An uncompressed bitset might use {} bits per value set {}", F.format(r.getSizeInBytes() * 8.0 / howmany));
	}
	...
	public void testSuperDense() {
		...
		r.runOptimize();
		LOG.info(
				"Bits used per value after run optimize = {}", F.format(r.getSizeInBytes() * 8.0 / r.getCardinality()));
		LOG.info("Bits used per gap after run optimize = {}", F.format(r.getSizeInBytes() * 8.0 / howmany));

		LOG.info("An uncompressed bitset might use {}", F.format(universe_size * 1.0 / r.getCardinality())
				+ " bits per value set");
	}
